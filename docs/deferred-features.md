...
| Ballot image upload | Omitted for archive demo | Ballot images can be sensitive, especially if marked or personally identifying. | Only if the team proves in-memory-only handling, no logs/storage, red-team tests, and a disable switch. |
| Ballot OCR / scan-to-contest extraction | Skipped for archive demo | OCR is unnecessary for the Measure D archive demo and would add privacy, accuracy, and testing burden. | Reconsider for a future real-election convenience layer after address/manual discovery are trusted. |
...
> Skip OCR and ballot image uploads for the archive demo. Keep contest selection
> manual, with official Santa Clara County ROV outbound links as fallback.
